...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user); logging_service.create_log(user, "用户登录"); return redirect('jobs:index')
            else: messages.error(request, "用户名或密码无效。")
        else: messages.error(request, "用户名或密码无效。")
    form = CustomAuthenticationForm(); return render(request, 'jobs/login.html', {'form': form})
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logging_service.create_log(user, "新用户注册并登录"); messages.success(request, "注册成功！")
            return redirect('jobs:index')
    else: form = CustomUserCreationForm()
    return render(request, 'jobs/register.html', {'form': form})
def logout_view(request):
    if request.user.is_authenticated:
        logging_service.create_log(request.user, "用户登出"); logout(request)
...
def job_detail_view(request, job_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save(); messages.success(request, f"职位 '{job.title}' 更新成功！")
...
def candidate_detail_view(request, candidate_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save(); messages.success(request, f"候选人 '{candidate.name}' 更新成功！")
            logging_service.create_log(request.user, "更新候选人", candidate)
            response = HttpResponse(status=204); response['HX-Refresh'] = 'true'; return response
    else: form = CandidateForm(instance=candidate)
...
def group_management_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            group = form.save(commit=False); group.user = request.user; group.save()
...
def compose_group_email_view(request, group_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            subject = form.cleaned_data['subject']; content = form.cleaned_data['content']
            from_account = form.cleaned_data['from_account']; created_count = 0
...
def email_log_detail_view(request, log_id):
    ...
    context = {'log': log, 'form': form}; return render(request, 'jobs/partials/email_log_detail.html', context)
@login_required
@require_POST
...
def cancel_email_task_view(request, log_id):
    ...
    if task.user == request.user or request.user.is_superuser:
        if task.status == EmailLog.EmailStatus.PENDING:
            task.status = EmailLog.EmailStatus.CANCELED; task.save()
            logging_service.create_log(request.user, "取消邮件任务", task.candidate)
            messages.success(request, f"已取消发送给 '{task.candidate.name}' 的邮件任务。")
        else: messages.warning(request, "该任务已发送或已取消，无法操作。")
    else: messages.error(request, "您无权操作此任务。")
    email_tasks = EmailLog.objects.select_related('user', 'from_account', 'candidate').all()
...
def add_template_view(request):
    ...
    return render(request, 'jobs/template_form.html', {'form': form, 'action': '创建'})
@login_required
@require_http_methods(["GET", "POST"])
def edit_template_view(request, template_id):
    ...
    return render(request, 'jobs/template_form.html', {'form': form, 'action': '编辑'})
@login_required
@require_POST
...
def _preview_content(request, form_class, content_type):
    ...
    uploaded_file = form.cleaned_data.get('file_upload'); text_content = form.cleaned_data.get('text_content'); texts = []
    if uploaded_file: texts = parsing_service.get_texts_from_file(uploaded_file)
    elif text_content: texts = [text_content.strip()]
    if not texts or not any(texts): return HttpResponse('<div class="alert alert-warning">未提取到任何有效文本。</div>', status=400)
    saved_providers = ApiKey.objects.filter(user=request.user).values_list('provider', flat=True)
    enabled_models = {key: model_info for key, model_info in settings.AI_MODELS.items() if model_info['provider'] in saved_providers}
    context = {'full_text': "\n\n---\n\n".join(texts), 'total_items_to_parse': len(texts), 'enabled_models': enabled_models, 'content_type': content_type}
...
def _parse_and_show(request, content_type, parse_function, template_name):
    ...
    if not full_text: messages.warning(request, "未提供任何文本内容进行解析。"); return render(template_name, {'parsed_items': []})
    ...
    context = {'parsed_items': parsed_results, 'content_type': content_type}; return render(request, template_name, context)
@login_required
@require_POST
def parse_and_show_jobs_view(request): return _parse_and_show(request, 'job', parsing_service.parse_multiple_job_descriptions, 'jobs/partials/job_preview_results.html')
@login_required
@require_POST
def parse_and_show_candidates_view(request): return _parse_and_show(request, 'candidate', parsing_service.parse_candidate_resume, 'jobs/partials/candidate_preview_results.html')
@login_required
@require_POST
def save_parsed_jobs_view(request):
    try:
        saved_count = 0; form_indexes = sorted(list(set([k.split('-')[1] for k in request.POST if k.startswith('form-')])))
        with transaction.atomic():
            for i in form_indexes:
                company, _ = Company.objects.get_or_create(name=request.POST.get(f'form-{i}-company_name', '未知公司'))
                job = Job.objects.create(
                    user=request.user, company=company, title=request.POST.get(f'form-{i}-title', ''), department=request.POST.get(f'form-{i}-department', ''),
                    salary_range=request.POST.get(f'form-{i}-salary_range', ''), level_set=[s.strip() for s in request.POST.get(f'form-{i}-level_set', '').split(',') if s.strip()],
                    locations=[s.strip() for s in request.POST.get(f'form-{i}-locations', '').split(',') if s.strip()], keywords=[s.strip() for s in request.POST.get(f'form-{i}-keywords', '').split(',') if s.strip()],
                    job_description=request.POST.get(f'form-{i}-job_description', ''), job_requirement=request.POST.get(f'form-{i}-job_requirement', ''), notes=request.POST.get(f'form-{i}-notes', ''),
                ); saved_count += 1
                logging_service.create_log(request.user, "批量创建职位", job)
        messages.success(request, f"成功保存 {saved_count} 条新职位！"); response = HttpResponse(status=204); response['HX-Refresh'] = 'true'; return response
    except Exception as e: messages.error(request, f"保存职位时发生严重错误: {e}"); return HttpResponse(status=500)
@login_required
@require_POST
