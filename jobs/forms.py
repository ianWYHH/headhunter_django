...
class JobForm(forms.ModelForm):
    ...
    class Meta:
        model = Job
        fields = ['title', 'company', 'department', 'salary_range', 'status', 'job_description', 'job_requirement', 'notes']
        labels = {'title': '职位名称', 'company': '公司', 'department': '所属部门', 'salary_range': '薪资范围', 'status': '职位状态', 'job_description': '职位描述', 'job_requirement': '职位要求', 'notes': '备注'}
        widgets = {'job_description': forms.Textarea(attrs={'rows': 5}), 'job_requirement': forms.Textarea(attrs={'rows': 5}), 'notes': forms.Textarea(attrs={'rows': 3})}
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def save(self, commit=True):
        self.instance.level_set = [s.strip() for s in self.cleaned_data.get('level_set_str', '').split(',') if s.strip()]
        self.instance.locations = [s.strip() for s in self.cleaned_data.get('locations_str', '').split(',') if s.strip()]
...
class JobParseForm(forms.Form):
    ...
    def clean(self):
        ...
        if not cleaned_data.get("file_upload") and not cleaned_data.get("text_content"): raise forms.ValidationError("请至少提供文件或文本内容中的一项。")
        return cleaned_data
...
class ApiKeyForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        provider_choices = sorted([(p_info['provider'], p_info['provider'].upper()) for p_key, p_info in settings.AI_MODELS.items()])
        unique_providers = sorted(list(set(provider_choices)))
        self.fields['provider'].choices = unique_providers
...
class CandidateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {'name': '姓名', 'homepage': '个人主页', 'github_profile': 'GitHub主页', 'linkedin_profile': '领英主页', 'external_id': '外部系统ID'}
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance and self.instance.pk:
...
class EmailAccountForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {'email_address': '邮箱地址', 'smtp_host': 'SMTP服务器', 'smtp_port': '端口', 'use_ssl': '使用SSL加密', 'is_default': '设为默认发件箱'}
...
class EmailComposeForm(forms.Form):
    ...
    content = forms.CharField(label="邮件内容", widget=forms.Textarea(attrs={'rows': 10}))
